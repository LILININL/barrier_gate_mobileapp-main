...
class FlutterRoundedDayPicker extends StatelessWidget {
  ...
  FlutterRoundedDayPicker({Key? key, required this.selectedDate, required this.currentDate, required this.onChanged, required this.firstDate, required this.lastDate, required this.displayedMonth, this.selectableDayPredicate, this.dragStartBehavior = DragStartBehavior.start, required this.era, this.locale, this.fontFamily, required this.borderRadius, this.style, this.customWeekDays, this.builderDay, this.listDateDisabled, this.onTapDay})
    ...
    return Padding(
      ...
      child: Column(
        children: <Widget>[
          Container(
            height: 45,
            decoration: BoxDecoration(color: style?.backgroundHeaderMonth, borderRadius: orientation == Orientation.landscape ? BorderRadius.only(topRight: Radius.circular(borderRadius)) : null),
