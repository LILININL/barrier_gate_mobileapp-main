...
class FlutterRoundedMonthPicker extends StatefulWidget {
  ...
  /// by [showDatePicker].
  FlutterRoundedMonthPicker({Key? key, required this.selectedDate, required this.onChanged, required this.firstDate, required this.lastDate, this.selectableDayPredicate, this.dragStartBehavior = DragStartBehavior.start, required this.era, this.locale, this.fontFamily, this.style, this.borderRadius = 0, this.customWeekDays, this.builderDay, this.listDateDisabled, this.onTapDay, this.onMonthChange})
...
class _FlutterRoundedMonthPickerState extends State<FlutterRoundedMonthPicker> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(color: widget.style?.backgroundPicker, borderRadius: orientation == Orientation.landscape ? BorderRadius.only(topRight: Radius.circular(widget.borderRadius)) : null),
